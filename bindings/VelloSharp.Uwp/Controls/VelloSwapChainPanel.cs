...
using VelloSharp.Uwp.Dispatching;
using VelloSharp.Uwp.Accessibility;
using VelloPaintSurfaceEventArgs = VelloSharp.WinForms.Integration.VelloPaintSurfaceEventArgs;
#if WINDOWS_UWP
using XamlSolidColorBrush = Windows.UI.Xaml.Media.SolidColorBrush;
#else
...
public sealed class VelloSwapChainPanel : SwapChainPanel, IDisposable, IVelloSwapChainPresenterHost, IVelloDiagnosticsProvider
{
    ...
    private readonly UwpSwapChainPanelSurfaceSource _surfaceSource;
    ...
    internal void RequestAccessKitAction(string action, ulong targetId)
    {
        ...
        if (DispatcherQueue is { HasThreadAccess: false } queue)
        {
            queue.TryEnqueue(() => RequestAccessKitAction(action, targetId));
            return;
        }

        if (Dispatcher is { HasThreadAccess: false } dispatcher)
        {
            _ = dispatcher.RunAsync(Windows.UI.Core.CoreDispatcherPriority.Normal, () => RequestAccessKitAction(action, targetId));
            return;
        }
#else
        if (DispatcherQueue is { HasThreadAccess: false } queue)
    ...
    public VelloSwapChainPanel()
    {
        ...
        _surfaceSource = new UwpSwapChainPanelSurfaceSource(this);
    ...
    {
        if (sender is not VelloSwapChainPanel panel || panel._isDesignMode)
        {
            return;
        }
    ...
    IVelloWindowsDispatcher? IVelloSwapChainPresenterHost.Dispatcher
    {
        get
        {
            ...
            return UwpDispatcher.Wrap(queue, Dispatcher);
#else
            return UwpDispatcher.Wrap(DispatcherQueue ?? Microsoft.UI.Dispatching.DispatcherQueue.GetForCurrentThread());
#endif
        }
    }
