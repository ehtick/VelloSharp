...
usage() {
  cat <<'EOF'
Usage: build-integrations.sh [options] [-- additional dotnet args]
...
supports_project() {
  ...
  case "${rel}" in
    integration/native/linux-*)
      [[ "${platform}" == "linux" ]] || return 1
      ;;
    integration/native/osx-*|integration/native/ios*)
      [[ "${platform}" == "macos" ]] || return 1
      ;;
    integration/native/win-*)
...
while IFS= read -r -d '' project; do
  projects+=("${project}")
done < <(find "${ROOT}/integration" -type f -name '*.csproj' -print0)

if [[ "${#projects[@]}" -eq 0 ]]; then
  echo "No integration projects found under integration/." >&2
  exit 1
fi

echo "Building integration projects (configuration: ${CONFIGURATION})"

for project in "${projects[@]}"; do
  supports_project "${project}" || {
    rel="${project#"${ROOT}/"}"
...
echo "Integration builds completed."
